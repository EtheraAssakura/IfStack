...
const preview = ref<string | null>(props.site.plan_path ? `/storage/${props.site.plan_path}` : null);
...
const removeImage = () => {
    if (confirm('Êtes-vous sûr de vouloir supprimer ce plan ?')) {
        router.delete(route('sites.remove-plan', props.site.id), {
            onSuccess: () => {
                form.plan = null;
                preview.value = null;
            }
...
const handleSubmit = () => {
    ...
    form.transform(data => ({
        ...data
    })).post(route('sites.update', props.site.id), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset();
            preview.value = null;
...
<template>
    <AppSidebarLayout :breadcrumbs="breadcrumbs">
        <Head>
            <title>Modifier {{ site.name }} - ISFAC</title>
        </Head>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="p-6">
                        <h1 class="text-2xl font-semibold mb-6">Modifier {{ site.name }}</h1>
                        ...
                        <form @submit.prevent="handleSubmit" class="space-y-6" enctype="multipart/form-data">
                            ...
                            <div class="flex justify-end gap-4">
                                <Button
                                    variant="outline"
                                    @click="router.visit(route('sites.index'))"
                                >
                                    Annuler
                                </Button>
                                <Button
